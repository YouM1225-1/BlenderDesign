# Official Blender MCP R18 Ruff-Scope Recovery Plan

## Goal and frozen admission

Adopt the already-correct two-file R15 source postimage without writing it again, run the never-started tests once, commit it once, construct a fresh R18 runtime, and perform one fresh complete live retest.

- Main HEAD is R17 Plan commit `e559ad0042ed3a862753e5a2aa51d4737bfca519`, parent `47503f7378b167cd7b8125c42ad8a384c69ff95e`. R17 Task2 report is dev/inode `16777232/307342813`, mode 0600/nlink1, size 2,408, SHA-256 `3c3d78b58a5dff698662b790f148fe50fb3a2a329bbdfecd22b9a90e16e2f849`, unique BLOCKED at the pinned Ruff gate because the frozen test preimage already has E402 at line 54; integration/build/gate/actual counts are zero. Its spec/quality/receipt inodes `307342814/307342815/307342816` are empty mode-0600/nlink1 files with SHA-256 `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`. R17 runtime/evidence/live paths are absent and immutable; R17 is never reopened or retried.
...
- Exact committed predecessor is R17 Plan `docs/superpowers/plans/2026-08-15-official-blender-mcp-r17-adoption-size.md`, 883/61,764/SHA `29cd8fb687694b23dd18e8ce4a6711fbc023c4cf1d2fb22c75bdb3f61d95fd99`. Exact committed R15 dependencies remain Plan `docs/superpowers/plans/2026-08-14-official-blender-mcp-r15-deferred-thumbnail.md`, 958/60,281/SHA `76f4ba731c95fb873cdd9beaec02bdab0bb1e488781578f145b6eb1ddb310e7c`; Appendix A replacements; Appendix E 346/14,230/SHA `ef81d9f1237873a3f8059f12066bbaedd976d67873440b39a058441ab37f16f0`; and Appendix C 126/4,629/SHA `d4e75498888c6f3c598e4547755a2e024b5c57fb4e1d4cbde1595359796ecbc0`.
...
- Task2 files are `.superpowers/sdd/modeling-remediation/r18-task2-report.md`, `.superpowers/sdd/modeling-remediation/r18-task2-spec-review.md`, `.superpowers/sdd/modeling-remediation/r18-task2-quality-review.md`, and `.superpowers/sdd/modeling-remediation/r18-task2-terminal-receipt.md`.
- Runtime is native-0700 `.superpowers/sdd/modeling-remediation/r18-live-runtime` with exactly `.superpowers/sdd/modeling-remediation/r18-live-runtime/r3_controller.py`, `.superpowers/sdd/modeling-remediation/r18-live-runtime/driver.sh`, `.superpowers/sdd/modeling-remediation/r18-live-runtime/r18_allocator.py`, and `.superpowers/sdd/modeling-remediation/r18-live-runtime/r18_allocator_runner.py`.
- Live paths are `.superpowers/sdd/modeling-remediation/r18-evidence/final-retest-r3/attempt-0001`, `.superpowers/sdd/modeling-remediation/task-7-r18-followup-1-brief.md`, `.superpowers/sdd/modeling-remediation/task-7-r18-followup-1-report.md`, `.superpowers/sdd/modeling-remediation/task-7-r18-followup-1-failure-review.md`, and `.superpowers/sdd/modeling-remediation/task-7-r18-followup-1-success-review.md`. Every path is absent at allocation; `.superpowers/sdd/modeling-remediation/r18-evidence/final-retest-r3/attempt-0002` is always forbidden.

## Task 1 — certify this Plan only

- Exact Appendix identities are A `79/3,916/f9df1ef2222a1c922d495494cce59ca84693f1b360395feb487a16f924ab6c12`; B `154/9,150/c2a760b04351e3aca41cd646de8cb3d61384cae8f2e9fbf138ad12cea3baf6b7`; C `347/21,693/629f0630a67764383fe44fbf85ad66d80ac65cdbee696d84b00cf95aecdf7afa`; D `135/9,243/84dae1ab7d64c14595e056b13a5c28d5b6f70ac92bc01c12cec54efaef6b4e37`; E `98/5,331/9ee2c1e16a7dfa3c46255eee0d39cc084a63e610f1185e9889ee74e0a0fb35d8`.
- Extract Appendices A/B/C/D/E exactly, require their declared identities, and run Appendix E once in a disposable native-0700 `/private/tmp` root; Appendix E owns the sole compile pass. It statically rejects the old two-path Ruff argv and binds the frozen-import/Python-3.14 syntax gate, invokes Appendix A in `dirty` mode, runs exact R15 E `--selfcheck`, builds with a synthetic non-parent 40-hex pin, gates under canonical Python 3.13, and requires one `R18_PLAN_HARNESS_GREEN adoption=1 primitives=1 builder=1 gate=1 static=1 ruff_scope=1 syntax=1`. It does not invoke Appendix C owner, R15 E real integration, allocator, runner, driver, Blender, MCP, or either source test.
- Three fresh reviewers audit one frozen Plan SHA for spec/safety, execution/state, and Ponytail/YAGNI. Their final files must each contain unique exact `CRITICAL: 0`, `IMPORTANT: 0`, and `MINOR: 0`; their respective unique final lines are `SPEC_VERDICT: PASS`, `EXECUTION_VERDICT: PASS`, and `PONYTAIL_VERDICT: PASS`. Any finding including Minor requires a fresh frozen round. Commit only this Plan with parent `e559ad0042ed3a862753e5a2aa51d4737bfca519`; record the Plan commit/SHA and each final review's canonical path plus exact dev/inode/size/SHA for the Appendix-C arguments and Task3 admission. Earlier FAIL reports are never eligible.
...
- Stage only the two paths, invoke Appendix A `staged`, and commit once with parent `4309a396...`. Require the commit tree changes exactly those two paths to the declared postimages, one parent, clean index/worktree/untracked state, and record `R18_SOURCE_PIN`. No amend/reset/rebase/second commit or further source write.
- Allocate/retain the native-0700 runtime parent FD. Extract exact R12 allocator and R14 runner into private native-0600 files. Invoke exact Appendix B once under canonical Python 3.13 as `builder.py R18_SOURCE_PIN OLD_CONTROLLER OLD_DRIVER OLD_ALLOCATOR OLD_RUNNER RUNTIME_ROOT`; require one `R18_RUNTIME_GREEN`. Invoke exact Appendix D once under the same canonical Python as `runtime_gate.py RUNTIME_ROOT R18_SOURCE_PIN OLD_CONTROLLER OLD_DRIVER OLD_ALLOCATOR OLD_RUNNER`; require one `R18_RUNTIME_GATE_GREEN`. Python 3.13 is `/Users/yeminjie/.local/share/uv/python/cpython-3.13.13-macos-aarch64-none/bin/python3.13`, dev/inode `16777232/259766870`, 0755/nlink1, size 17,439,616, SHA `7fc33c67b5d5d91b3ce9ab16d2a354ad623f438c24eed3f543a57fc635aea683`.
...
- Admission binds: R18 Plan commit/SHA and exact final three Plan-review identities/zero findings; upstream parent `4309...`, exact `R18_SOURCE_PIN`, exact two-file tree and clean status; R18 Task2 report unique PASS, both Task2 approvals, matching preallocated receipt inode/content native 0400/nlink1; exact runtime parent/four leaves; frozen R17 BLOCKED/empty files/absences; all fresh live absences; empty ports/process inventory. It also binds immutable driver-input `.superpowers/sdd/modeling-remediation/task-7-brief.md` as dev/inode `16777232/295274948`, uid 501, mode 0600, nlink1, size 538,571, SHA `fd27537dc55d460403f1bdb6d2af5300820df08ecb722d204e017d3fa3f9ba6b`, exactly matching the unchanged R14/R18 driver constants.
- After that exact R18 admission, inherit the committed R17 Task3 transitions verbatim, substituting only R17 Plan/review/Task2/source/runtime/live namespaces and markers with the bound R18 values. This includes report-first original-FD fallback, one fresh GUI preflight and action-time Online Access confirmation, raw `bpy.app.tempdir`, exact R10 scratch checker once, exact R18 runner once, retained evidence/controller FDs, exact ten-argument R18 driver once in one PTY with FD8, at most one ticket/actual run, and cleanup on every terminal. The ten arguments retain the exact R17 order; no argument is reconstructed from prose.
- `.superpowers/sdd/modeling-remediation/task-7-r18-followup-1-brief.md` is a fresh root-only GUI/scratch/admission/review record and is never presented as driver input. The unchanged driver intentionally consumes only the separately bound immutable `.superpowers/sdd/modeling-remediation/task-7-brief.md`; Task3 binds both artifacts and never substitutes or confuses their roles.
- Display every fresh R18 PNG in manifest order, collect one explicit verdict per image, and send exactly one canonical ordered Option-C ACK. No predecessor image/verdict/ACK, scratch, ticket, listener, PTY, attempt, or report is reusable.
...
- Successful first-parent/path-array history has exact length 17: runbook, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16, R17, R18, audit; parents are old-R3, runbook, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16, R17, R18.
...
def main() -> int:
    ...
    print(f"R18_ADOPTION_GREEN mode={mode} tool=1 test=1 diff=1 index=1 untracked=0")
    return 0
...
def main() -> int:
    ...
    print("R18_PLAN_HARNESS_BEGIN", flush=True)
    adoption, builder, owner, gate = map(Path, sys.argv[1:])
    expected = {
        adoption: (79, 3916, "f9df1ef2222a1c922d495494cce59ca84693f1b360395feb487a16f924ab6c12"),
        builder: (154, 9150, "c2a760b04351e3aca41cd646de8cb3d61384cae8f2e9fbf138ad12cea3baf6b7"),
        gate: (135, 9243, "84dae1ab7d64c14595e056b13a5c28d5b6f70ac92bc01c12cec54efaef6b4e37"),
        owner: (347, 21693, "629f0630a67764383fe44fbf85ad66d80ac65cdbee696d84b00cf95aecdf7afa"),
    }
    checked = {}
    for path, (lines, size, digest) in expected.items():
        ...
        if raw.count(b"\n") != lines or len(raw) != size or hashlib.sha256(raw).hexdigest() != digest:
            raise RuntimeError(f"R18 appendix differs: {path}")
        checked[path] = raw
    owner_raw = checked[owner]
    if owner_raw.count(b"str(SOURCE / TOOL), str(SOURCE / TEST)") != 0 or owner_raw.count(b'"E4,E7,E9,F", str(SOURCE / TOOL)]') != 1 or owner_raw.count(b"frozen_import = b\"from tests.mcp_client import MCPClient\"") != 1 or owner_raw.count(b"R18_TEST_SYNTAX_GREEN") != 2:
        raise RuntimeError("R18 Ruff/syntax owner anchors differ")
    run([PY313, str(adoption), "dirty"], "R18_ADOPTION_GREEN mode=dirty ")
    tmp = Path(tempfile.mkdtemp(prefix="r18-plan-", dir="/private/tmp"))
    os.chmod(tmp, 0o700)
    ...
    try:
        ...
        run([PY313, str(builder), SYNTHETIC_PIN, str(old_controller), str(old_driver), str(allocator), str(runner), str(runtime)], "R18_RUNTIME_GREEN ")
        run([PY313, str(gate), str(runtime), SYNTHETIC_PIN, str(old_controller), str(old_driver), str(allocator), str(runner)], "R18_RUNTIME_GATE_GREEN ")
    finally:
        os.close(root_fd)
        shutil.rmtree(tmp)
    print("R18_PLAN_HARNESS_GREEN adoption=1 primitives=1 builder=1 gate=1 static=1 ruff_scope=1 syntax=1", flush=True)
    return 0
...
PLAN_REL = Path("docs/superpowers/plans/2026-08-15-official-blender-mcp-r18-ruff-scope.md")
R17_REL = Path("docs/superpowers/plans/2026-08-15-official-blender-mcp-r17-adoption-size.md")
...
R17 = "e559ad0042ed3a862753e5a2aa51d4737bfca519"
...
def main() -> int:
    ...
    report = MODELING / "r18-task2-report.md"
    spec = MODELING / "r18-task2-spec-review.md"
    quality = MODELING / "r18-task2-quality-review.md"
    receipt = MODELING / "r18-task2-terminal-receipt.md"
    runtime = MODELING / "r18-live-runtime"
    fds: dict[str, int] = {}
    ...
    try:
        ...
        if sh(MAIN, "/usr/bin/git", "rev-parse", "HEAD").stdout.strip() != plan_commit or sh(MAIN, "/usr/bin/git", "rev-list", "--parents", "-n", "1", "HEAD").stdout.split() != [plan_commit, R17]:
        ...
        r17_plan = subprocess.check_output(["/usr/bin/git", "-C", str(MAIN), "show", f"{R17}:{R17_REL}"])
        if (r17_plan.count(b"\n"), len(r17_plan), hashlib.sha256(r17_plan).hexdigest()) != (883, 61764, "29cd8fb687694b23dd18e8ce4a6711fbc023c4cf1d2fb22c75bdb3f61d95fd99"):
            raise RuntimeError("R17 Plan differs")
        exact_file(Path(__file__), 0o600, int(owner_size_raw), owner_sha)
        exact_file(adoption, 0o600, 3916, "f9df1ef2222a1c922d495494cce59ca84693f1b360395feb487a16f924ab6c12")
        exact_file(builder, 0o600, 9150, "c2a760b04351e3aca41cd646de8cb3d61384cae8f2e9fbf138ad12cea3baf6b7")
        exact_file(gate, 0o600, 9243, "84dae1ab7d64c14595e056b13a5c28d5b6f70ac92bc01c12cec54efaef6b4e37")
        ...
        r17_report = MODELING / "r17-task2-report.md"
        r17_raw, r17_report_id = read_exact(r17_report)
        if r17_report_id != (16777232, 307342813, 2408, "3c3d78b58a5dff698662b790f148fe50fb3a2a329bbdfecd22b9a90e16e2f849") or r17_raw.splitlines().count(b"STATUS: BLOCKED") != 1:
            raise RuntimeError("R17 Task2 report differs")
        r17_empty_ids = []
        for name, inode in (("r17-task2-spec-review.md", 307342814), ("r17-task2-quality-review.md", 307342815), ("r17-task2-terminal-receipt.md", 307342816)):
            ...
            if identity != (16777232, inode, 0, hashlib.sha256(b"").hexdigest()):
                raise RuntimeError(f"R17 empty artifact differs: {name}")
            r17_empty_ids.append(identity)
        ...
        absent = [
            MODELING / "r17-live-runtime",
            MODELING / "r17-evidence",
            MODELING / "task-7-r17-followup-1-brief.md",
            MODELING / "task-7-r17-followup-1-report.md",
            MODELING / "task-7-r17-followup-1-failure-review.md",
            MODELING / "task-7-r17-followup-1-success-review.md",
        ...
        adoption_dirty = marker([PY313, str(adoption), "dirty"], "R18_ADOPTION_GREEN mode=dirty ")
        c_raw = body(r15, b"## Appendix C ", 126, 4629, "d4e75498888c6f3c598e4547755a2e024b5c57fb4e1d4cbde1595359796ecbc0")
        tmp = Path(tempfile.mkdtemp(prefix="r18-task2-", dir="/private/tmp"))
        os.chmod(tmp, 0o700)
        tmp_fd = os.open(tmp, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW)

        bindings = (
            f"r18_plan_commit: {plan_commit}\nr18_plan_sha256: {plan_sha}\n"
            + "".join(f"r18_plan_review_{path.name}: {identity!r}\n" for path, identity in zip(review_paths, plan_reviews))
            + f"r17_task2_report: {r17_report_id!r}\nr17_empty_artifacts: {r17_empty_ids!r}\n"
            + f"base_driver_brief: {base_brief_id!r}\n"
            + f"r18_dirty_adoption_marker: {adoption_dirty.strip()}\n"
            + "r18_tool_postimage_sha256: 85b1c57accf3427ca43259ea2f5b50d9a4fc95b1741096cb5e7050b052678f26\n"
            + "r18_test_postimage_sha256: a8912677324da140d707e604b7e4ed6dc28b0ff4dacbb0ae8cc24ba8e6f05c63\n"
            + "r18_binary_diff_sha256: a499937290390411db62ed7b1a6a3757357ff627feeca7a08cf9ccd7dd40d248\n"
            + "r17_task2_status: BLOCKED\n"
        ).encode()
        ...
        marker([PY314, "-c", "import sys; from pathlib import Path; compile(Path(sys.argv[1]).read_bytes(), sys.argv[1], 'exec'); print('R18_TEST_SYNTAX_GREEN')", str(SOURCE / TEST)], "R18_TEST_SYNTAX_GREEN")
        ...
        marker([PY313, str(adoption), "staged"], "R18_ADOPTION_GREEN mode=staged ")
        ...
        old_driver = MAIN / ".superpowers/sdd/modeling-remediation/r14-live-runtime/driver.sh"
        builder_count = 1
        build_output = marker([PY313, str(builder), source_pin, str(old_controller), str(old_driver), str(allocator), str(runner), str(runtime)], "R18_RUNTIME_GREEN ")
        gate_count = 1
        gate_output = marker([PY313, str(gate), str(runtime), source_pin, str(old_controller), str(old_driver), str(allocator), str(runner)], "R18_RUNTIME_GATE_GREEN ")
        ...
        os.fsync(fds["report"])
        print(f"R18_TASK2_PHASE1_GREEN source_pin={source_pin} actual_run_count=0", flush=True)
        ...
        print("R18_TASK2_TERMINAL PASS", flush=True)
        return 0
...
## Appendix D — exact matching R18 runtime gate
...
R17_COMMIT = "e559ad0042ed3a862753e5a2aa51d4737bfca519"
...
def main() -> int:
    if len(sys.argv) != 7:
        raise SystemExit("usage: runtime_gate.py RUNTIME_ROOT R18_SOURCE_PIN OLD_CONTROLLER OLD_DRIVER OLD_ALLOCATOR OLD_RUNNER")
    ...
    if names != ["driver.sh", "r18_allocator.py", "r18_allocator_runner.py", "r3_controller.py"]:
    ...
    allocator, _ = owned(root / "r18_allocator.py", 0o600)
    runner, _ = owned(root / "r18_allocator_runner.py", 0o600)
    ...
    restored_driver = one(restored_driver, b'docs/superpowers/plans/2026-08-15-official-blender-mcp-r18-ruff-scope.md', b'docs/superpowers/plans/2026-08-14-official-blender-mcp-r14-allocator-args.md')
    restored_driver = one(restored_driver, b'task-7-r18-followup-1-report.md', b'task-7-r14-followup-1-report.md')
    restored_driver = one(restored_driver, b'.superpowers/sdd/modeling-remediation/r18-evidence/final-retest-r3/attempt-0001', b'.superpowers/sdd/modeling-remediation/r12-evidence/final-retest-r3/attempt-0001', 2)
    restored_driver = one(restored_driver, controller_sha.encode(), OLD_CONTROLLER_SHA.encode())
    new_topology = f'  test "$(git -C "$FEATURE_ROOT" rev-parse "$R3_PLAN_COMMIT^")" = {R17_COMMIT}\n  test "$(git -C "$FEATURE_ROOT" rev-parse {R17_COMMIT}^)" = 47503f7378b167cd7b8125c42ad8a384c69ff95e\n  test "$(git -C "$FEATURE_ROOT" rev-parse 47503f7378b167cd7b8125c42ad8a384c69ff95e^)" = 6e78a91d9c4fab6746262c88e7032e789a3bae1c\n  test "$(git -C "$FEATURE_ROOT" rev-parse 6e78a91d9c4fab6746262c88e7032e789a3bae1c^)" = deb5426a57748ae4e89e91afe0a1752e4c9fc2b0\n  test "$(git -C "$FEATURE_ROOT" rev-parse deb5426a57748ae4e89e91afe0a1752e4c9fc2b0^)" = c0f38156cad28996cfddcabb1cf775ae84983cf5\n'.encode()
    ...
    restored_allocator = one(restored_allocator, b'PARTS = ("r18-evidence", "final-retest-r3", "attempt-0001")', b'PARTS = ("r12-evidence", "final-retest-r3", "attempt-0001")')
    if restored_allocator.count(b"R18") != 11:
        raise RuntimeError("allocator diagnostic count differs")
    restored_allocator = restored_allocator.replace(b"R18", b"R12")
    ...
    restored_runner = one(restored_runner, b'R18_ALLOCATION_GREEN', b'R12_ALLOCATION_GREEN')
    if restored_runner.count(b"R18") != 15:
        raise RuntimeError("runner diagnostic count differs")
    restored_runner = restored_runner.replace(b"R18", b"R14")
    if restored_runner != old_runner:
        raise RuntimeError("runner reversal differs")

    for raw, path in ((controller, root / "r3_controller.py"), (allocator, root / "r18_allocator.py"), (runner, root / "r18_allocator_runner.py")):
        compile(raw, str(path), "exec")
    ruff = subprocess.run(["/Users/yeminjie/.local/bin/uvx", "--quiet", "ruff@0.16.2", "check", "--no-cache", "--isolated", "--target-version", "py313", "--select", "E4,E7,E9,F", str(root / "r3_controller.py"), str(root / "r18_allocator.py"), str(root / "r18_allocator_runner.py")], text=True, capture_output=True, timeout=60)
    if ruff.returncode:
    ...
    for number, body in enumerate(heredocs, 1):
        compile(body, f"<r18-heredoc-{number}>", "exec")
    ...
    print("R18_RUNTIME_GATE_GREEN controller=1 driver=1 allocator_equivalent=1 runner_equivalent=1 probe=1 static=1")
    return 0
...
## Appendix B — exact direct R18 runtime builder

Extract this 154-line body without fences. Invoke once as specified in Task2. It is the exact R15 Appendix B algorithm with only the declared R18 literals and five-line topology.
...
def main() -> int:
    ...
    try:
        ...
        driver = replace(driver, b'docs/superpowers/plans/2026-08-14-official-blender-mcp-r14-allocator-args.md', b'docs/superpowers/plans/2026-08-15-official-blender-mcp-r18-ruff-scope.md')
        driver = replace(driver, b'task-7-r14-followup-1-report.md', b'task-7-r18-followup-1-report.md')
        driver = replace(driver, b'.superpowers/sdd/modeling-remediation/r12-evidence/final-retest-r3/attempt-0001', b'.superpowers/sdd/modeling-remediation/r18-evidence/final-retest-r3/attempt-0001', 2)
        ...
        allocator = replace(allocator, b'PARTS = ("r12-evidence", "final-retest-r3", "attempt-0001")', b'PARTS = ("r18-evidence", "final-retest-r3", "attempt-0001")')
        if allocator.count(b"R12") != 11:
            raise RuntimeError("allocator diagnostic anchors differ")
        allocator = allocator.replace(b"R12", b"R18")
        create(root_fd, root, root_fields, created, "r18_allocator.py", allocator)
        allocator_sha = hashlib.sha256(allocator).hexdigest()
        ...
        runner = replace(runner, b'R12_ALLOCATION_GREEN', b'R18_ALLOCATION_GREEN')
        if runner.count(b"R14") != 15:
            raise RuntimeError("runner diagnostic anchors differ")
        runner = runner.replace(b"R14", b"R18")
        create(root_fd, root, root_fields, created, "r18_allocator_runner.py", runner)

        print(f"R18_RUNTIME_GREEN source_pin={pin} controller_sha256={controller_sha} driver_sha256={hashlib.sha256(driver).hexdigest()} allocator_sha256={allocator_sha} runner_sha256={hashlib.sha256(runner).hexdigest()}")
        return 0
    except BaseException:

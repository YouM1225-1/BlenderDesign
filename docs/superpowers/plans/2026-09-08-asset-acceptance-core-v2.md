...
- `docs/README.md`：将“现行规范与技术决策”中的当前入口切换为 V5，删除重复的 V5/v2 迁移目标入口，保留可追溯的 V3.8 历史链接；更新 schema v1 和切换前状态说明。
- 对应计划与 `docs/README.md` 的计划索引：标记 M0/M1 实际完成的任务及验证结果；M2/M3 继续保留为待实施，不把原型结果当作完成证据。
- `docs/architecture.md` 与根 `README.md`：更新 schema v2、冻结输入、证据归属及 CLI 的实际边界；明确 R2–R4 worker 尚未接线、通用资产仍不能自动放行。

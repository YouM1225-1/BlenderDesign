...
- **状态文档大小**：journal 为每个候选保存完整镜像，一个 runtime 镜像约 2 MB。安装器状态 JSON 的读写、身份证据读取与 runtime 启动闸门读取 receipt 共用 32 MiB 上限；写入前先检查，超限时在任何文件变更之前失败，因此清理意图写不下时在删除边界之前停止，不会发布读不回的 journal 或 receipt。旧版本安装器的读取上限为 16 MiB，读不了更大的 journal，只会失败关闭。journal 大小仍随候选数线性增长，约 15 个 runtime 镜像达到上限；届时需另行把完整镜像移出 journal，只保留摘要。

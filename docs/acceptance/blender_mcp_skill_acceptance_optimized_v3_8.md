# Blender MCP / Skill 建模产物验收方案 V3.8
...
`visual_unverified` 是一条 **finding code**,`severity="error"`:表示图像已生成但未被任何判定器实际打开判读。它按 §2.5 第二步走 `Fail` 分支,**不产生 `NotTested`**。P0 的"判读"主体是 oiiotool 或确定性比较器,不是人;人工审阅记录另存于 §8.3,不改变机器判定。

**`NotTested` 的唯一产生点(冻结)**:coordinator 在**编排阶段**因前序 stage 已失败而**未启动**某 stage 时,为该 stage 的全部非 N/A check 合成 `NotTested`(例如 R2 失败后不再跑 R3/R4)。除此之外 §2.5 的聚合函数不产生该状态。它同样按 §2.6 条款 7 阻断,故"提前中止"不会被误读为通过。
...
第二步 —— 由 coordinator 聚合出 check.raw_status(冻结优先级,自上而下首个命中):
  NotApplicableByContract  当 check_id ∈ R0 的 N/A 集
             —— 该记录由 coordinator 按 §2.7 合成,**不经过下面任何一行**;
                若它同时出现在某个 result 的 checks[] 中,则是伪造 → forged_not_applicable
  Crash      当该 check 的 writer 子进程异常终止(信号终止,或退出后无任何输出)
  Missing    当 writer 已正常结束但未产出其 result 文件,或该文件中缺少该 check 的记录
  Truncated  当 check.source_truncated == true,或其证据文件达大小上限
  Fail       当存在 severity == "error" 的 finding
  Warning    当存在 severity == "warning" 且 disposition == None 的 finding
  Pass       其他一切(findings 为空,或全部 warning 均为 AcceptedWarning,info 不影响)
             —— 若原本存在至少一条 warning 且全部被接受,同时置 check.accepted = true

**顺序的两条理由(冻结)**:① N/A 必须最先命中——否则被跳过 stage 的 check 因"无人写出记录"落入 `Missing`,而 §2.6 条款 7 对 `Missing` 一律阻断,结果是**任何资产都永不放行**;② `Crash` 必须先于 `Missing`——被信号杀死的子进程本就不会留下 result 文件,若 `Missing` 在前则 `Crash` 近乎不可达,夹具 `tool_killed_by_signal` 期望的 `tool_crashed` 会变成 `evidence_missing`。
...
**未知字段处置(冻结)**:`result.json` 的顶层与 `checks[]` 元素均**封闭**;出现未知键一律 `tool_output_invalid`。**唯二例外**是保留键名 `raw_status` 与 `disposition`——它们出现时归 `forged_disposition`(而非 `tool_output_invalid`),因为它们表达的是"子进程试图代行 coordinator 的判定",与普通的 schema 打字错误性质不同。这条例外使夹具 `forged_disposition_written` 有确定的预期二元组。

**`raw_status` 与 `disposition` 均由 coordinator 独占计算(冻结)**:子进程**只写 `findings[]`**(每条含 `code`/`severity`/`pointer`/`offset`/`detail`)与 `source_truncated`,**不写 `raw_status`,也不写 `disposition`**。这是本规范的**统一表达形式**:

> **每个 check 的结果就是一组 finding。没有 finding = 通过;有 error finding = 失败;有 warning finding = 视 allowlist 而定。**

因此像 `r3.export.file_nonempty` 这类"非布尔即真"的检查,通过时写零条 finding、失败时写一条 `severity="error"` 的 finding(`code` 取 §7.1 注册表为该 check 声明的码,未声明时用 `check_failed_generic`)。终结态 `Missing`/`Crash`/`Truncated` 由 coordinator 依进程与文件状态判定,子进程无从伪造;`NotApplicableByContract` 由 coordinator 按 §2.7 合成。子进程产物中出现 `raw_status` 或非 null `disposition` 字段 → `forged_disposition` Fail(§7.2)。

理由与 §2.6 条款 6 防伪造 N/A 对称:allowlist 是合同的一部分,而合同按 §1 是 Verifier 侧资产,任何依赖合同的判定都必须在 Verifier 侧完成。原始证据(逐条 finding 的 `code`/`severity`)全程不被改写,`raw_status` 是对它们的**确定性函数**,不是可被覆盖的独立事实。

`accepted` 与 `effective_status` 都是 summary 中的**派生序列化字段**(§7.3),写出以便审阅方不重算即可查看;`raw_status` 与逐条 finding 原样保留,不被改写。

**四元组分量来源**:`check_id` 取自 §7.1 注册表;`tool_id` 取自 §7.4 工具锁定表的 `id` 列;`tool_version` 取自该表实测记录值;`warning_code` 对外部工具取其原生稳定码(glTF-Validator 的消息码),对自研检查取 §7.1 注册表 `warning_codes` 列声明的码——**未在注册表声明的自研 warning code 不可 allowlist**(fail-closed)。自研检查的 `tool_id` 恒为 `acceptance`、`tool_version` 为该包内容哈希版本(§7.4 有对应行),因此注册表 `warning_codes` 列声明的全部自研码——当前为 `unsupported_datablock_type`、`unsupported_modifier_type`、`empty_material_slot`、`packed_dependency`、`budget_near_limit`、`non_triangle_primitive` 六个——**都可以被 allowlist**(以注册表为准,本处列举仅供参考;新增码必须先入注册表);`acceptance` 版本变化会使既有 allowlist 条目失效并需重新审批,这是有意的。
...
整体放行(coordinator exit 0)当且仅当以下全部成立:
 ...
 9. 所有子进程 exit == 0;**所有 result 文件**(含 coordinator 自产的 `r3_result`/`r4_result`)schema 合法且 `success == true`
...
`blend_native` 的 N/A 集 = §7.1 中"适用 kind = interchange"的 **13 项**(其中 R3 的 7 项构成整段跳过的 stage,R4 的 5 项与该 stage 内其他 check 并存);`interchange` 的 N/A 集 = "适用 kind = blend_native"的 **3 项**(均在 R4)。注册表合计 **37 项 = 21 all + 13 interchange + 3 blend_native**。
...
instances: [InstanceRecord]     # 按 (instancer_stable_id, persistent_id, source_object_stable_id) 升序
                                #   三分量构成全序:真实对象共享 ("", []) 前两分量,由第三分量区分(渲染级)
...
| `InstanceRecord` | `instancer_stable_id: str`(真实对象取空串 `""`,实例取其 instancer 的 stable_id)、`persistent_id: [int]`(真实对象取 `[]`)、`source_object_stable_id`、`matrix_world`(16 个量化 float)、`geometry_digest`(引用其 source 的 `GeometrySummary.evaluated`,不重复展开)。<br>**两套过滤集(冻结,修正 V3.8 初稿的"三方一致"错误)**——它们服务于不同目的,不得混用:

- **渲染集 `render_set`**(供 §5.1 包围盒与渲染使用):遍历 `depsgraph.object_instances`,保留 `object.type ∈ {MESH, CURVE, SURFACE, META, FONT, VOLUME, POINTCLOUD, GREASEPENCIL}` 且**该 instance 最终会被渲染**(`object.hide_render == False`,且其所属 collection 链上无 `hide_render`/`exclude`)。口径是"Blender 会画出什么"。
- **导出对账集 `export_set`**(供 `p01_object_count` / `p02_triangle_count` 使用):在 `render_set` 基础上再加两条——① 类型必须是 glTF 可表示的 `{MESH, CURVE, SURFACE, META, FONT}`(`VOLUME`/`POINTCLOUD`/`GREASEPENCIL` 在 glTF 中无对应表示,导出器不产生 node,计入即必然误拒);② `object.visible_get() == True`(视口可见性),以匹配 preset 的 `use_visible=True`——eye-icon 隐藏但 `hide_render=False` 的对象**会被渲染却不会被导出**,若不加此条即构成第六处误拒。

`ObjectRecord.visible_viewport` 与 `CollectionRecord.exclude`/`hide_render` 正是为这两套集合的可复算而记录的。manifest 同时输出 `render_set` 与 `export_set` 两个 `stable_id` 列表,使两侧对账口径显式可核。

**若 `artifact_kind == interchange` 而 `export_set` 为空**,`r3.export.file_nonempty` 写一条 `severity="error"`、`code="no_exportable_geometry"` 的 finding(合同不应为纯点云/体积资产声明 interchange)。<br>**`objects[]` 是 datablock 级视图,`instances[]` 是渲染级视图;`p01_object_count` / `p02_triangle_count` 一律以 `instances[]` 为准**——否则 collection-instance 资产的 source 侧只有 1 条记录而 GLB 侧有 N 个 node,必然误拒 |
...
**coverage 完整性算法(冻结)**:按 §4.2 逐类型枚举 `bpy.data` 的具体集合(**排除 `all_ids`**),`coverage.total` = 各类型计数之和;`enumerated_types` 为已建记录的类型,`skipped_types` 为已知不参与判定的类型(`screens`/`workspaces`/`window_managers`/`brushes`/`palettes`)。`r2.inventory.coverage_complete` 的判定:出现**未分类的非空类型**时,若该类型在合同 `tolerated_unknown_types` 中则写一条 `severity="warning"`、`code="unsupported_datablock_type"` 的 finding(从而可被 allowlist),否则写 `severity="error"` 的同名 finding(Fail)。这给了该 warning code 唯一的产生点,消除死 allowlist 条目。
...
**待实测项登记(P0 第一阶段必须完成,不阻塞编码但阻塞 §7.8 完成定义)**:

| 待测 | 影响 |
|---|---|
| Workbench 在同进程两次渲染是否逐位一致 | `r4.visual.self_determinism` 的零容差是否成立(见下) |
| `use_visible=True` 的实际谓词是否等同 `object.visible_get()` | `export_set` 定义(§4.1)与 `p01` 是否两端一致 |
| collection 级 `hide_render`/`exclude` 对导出的影响 | 同上 |
| `VOLUME`/`POINTCLOUD`/`GREASEPENCIL` 的实际导出行为 | `export_set` 的类型集是否需再收窄 |
| collection-instance 导出为 N 个 node 时 `extras.bcx_uid` 是否逐 node 传递 | `p09` 多重集比较与夹具 `collection_instance_roundtrip` 能否变绿 |

以上五项各自的实测结论必须写回本文并附命令与输出;未完成前对应夹具标 `assumption_unverified`。

**未经实测的承重假设(诚实标注)**:`r4.visual.self_determinism` 使用零容差(`--fail 0 --failpercent 0`),前提是"Workbench 在同一进程内两次渲染逐位一致"。§5 的其余关键参数均有本机实测支撑,**唯独这一条只有定性论断**。P0 第一步必须先做一次专项实测(同场景连渲两次比对字节);若 Metal 后端的 Workbench 非逐位一致,则把该 check 的容差改为由合同 `visual_thresholds` 的同一平台键提供(而非硬编码 0),并在文档记录实测值。在实测完成前,该 check 在其 `metrics` 中置 `"assumption_unverified": true`(与 `"hashes"` 并列)(§7.3 的 `metrics` 是结构化证据字段,无需新增 schema 字段),并在 `summary.advisories[]` 追加一条同名 advisory 以便审阅方一眼看到。
...
1. `r4.visual.all_views_rendered`:按 kind 的期望张数全部存在、非零字节、可被 oiiotool 读取;任一不满足即写一条 `severity="error"`、`code="view_missing_or_unreadable"` 的 finding——`blend_native` = 32(source 侧 8 视角 × 4 pass);`interchange` = 48(source 侧 32 + import 侧 16)。差异图不计入本 check,由 `r4.visual.source_import_match` 产出并由 §2.6 条款 4 的 file 集校验;
2. `r4.visual.self_determinism`:同一 run 内对 source 侧的 `clay`/`silhouette`/`wire` 共 24 张各重渲一次(第二组渲染到 `views2/` 子目录),与首次结果按 `oiiotool --fail 0 --failpercent 0 --diff` 逐张比较,**任一张不一致即该 check Fail**(单条 check 聚合 24 张结果)。第二组图像**不进入 evidence file ID 集**(条款 4 只覆盖 `views/` 的 32 张 + 差异图),但其 24 个 SHA-256 以 `metrics = {"hashes": {view_pass: [sha_first, sha_second]}}` 写入该 check 的 `metrics`(与 §5.1 的 `assumption_unverified` 同处一个 object,互不覆盖)(§7.3;`detail` 只放单行原因),使判定可复核而不使 file 集随渲染次数漂移;
...
失败态映射(与 Blender 上游术语对齐,不新增状态):渲染子进程崩溃 → 该 stage 的 writer 崩溃,按 §2.5 第二步得 `Crash`;**图像文件缺失或零字节** → 由 §2.6 条款 4 的 file 集校验捕获(`evidence_missing`),同时 `r4.visual.all_views_rendered` 收到一条 `severity="error"` 的 finding 而得 `Fail`;像素比较超阈值 → 对应 check 收到 error finding 而得 `Fail`。**所有判定都经由"写 finding → coordinator 聚合"这一条路径,不存在第二个状态产生处**。
...
`interchange/glb` 的 source↔import 比较只在下表声明的字段上进行。表中未出现的字段不参与判定。**判定分工(冻结)**:`preserved` 类由 `r4.projection.preserved_fields_match` 承接——发生任何差异即 Fail;`transformed` 类由 `r4.projection.transformed_within_tolerance` 承接——超出本表声明的变换关系或容差即 Fail(`p04` 比值越界、`p07` 超 1e-3、`p13` 换算不符均由它捕获);`p09` 由 `r4.projection.ambiguous_object_names` 承接;`lost` 类不比较。**没有任何比较字段处于无 owner 状态**;**未被合同 `projection.lost` 列出的字段(即 preserved/transformed 两类)若实际丢失,记 `r4.projection.undeclared_loss` Fail**——即"丢失"本身不是问题,"未预先声明的丢失"才是。合同 `projection` 三个数组的并集必须恰好等于本表 **14 行**(`p01`…`p14`),缺项、多项或未知 ID → `contract_invalid`。

**source 取向(冻结,修正第四处误拒)**:§7.4 的导出 preset 固定 `export_apply=True`,GLB 一侧**恒为 evaluated 几何**。因此所有几何类字段(`p01`–`p06`)的 source 侧**一律取 §4.1 `GeometrySummary.evaluated`**,不取 `authored`。若取 authored,带 `SUBSURF`(顶点约 ×16)、`MIRROR`/`ARRAY`/`SOLIDIFY`(包围盒改变)的**合法资产会被必然误拒**——而这四类恰在 §4.1 冻结的 modifier 注册表内,即本规范预期它们出现。`p11_modifier_stack` 与 `p14_drivers_constraints` 取 authored(它记录的正是"导出前有哪些 modifier"),且分类为 `lost`、不参与比较。标"—"的字段与 authored/evaluated 之分无关。
...
| `p09_object_identity` | object 身份对应 | — | transformed | **不依赖名称**:R3 导出前由 `export_glb.py` 在**候选副本**上为每个源 object 写入自定义属性 `bcx_uid`(值为 R2 manifest 的 `stable_id`),`export_extras=True` 使其进入 glTF `extras`,导入后由 §4.1 的 `ObjectRecord.bcx_uid` 字段回连。**实测确认该机制可用**(Blender 5.2:导出后原始 GLB 的 JSON chunk 含 `extras:{bcx_uid:...}`,独立进程导入后可从对象自定义属性读回)。<br>**防伪造与实例化(冻结)**:R2 阶段若候选自身已存在名为 `bcx_uid` 的自定义属性 → `r2.inventory.no_reserved_props` Fail(该键名为 evaluator 保留)。R4 阶段的判定基于**多重集**而非"唯一":从 source 侧 `instances[]` 计算期望多重集 `E`——每个可渲染 object 的 `stable_id` 出现次数 = 它在 `instances[]` 中的实例数(未被实例化的对象计 1 次);import 侧收集实际多重集 `A`。`A == E` 为 Pass,否则 `r4.projection.ambiguous_object_names` Fail。**这使被实例化 N 次的对象合法地携带 N 个相同 uid**——V3.7 的"重复即 Fail"会把 `instances[]` 本要保护的 collection-instance 资产变成第五处误拒,已修正。缺失 uid(期望存在却没有)仍是 Fail。<br>**退化路径**:目标 glTF 剖面禁止 extras 时退化为名称匹配,此时必须先检测归一化碰撞(`Cube` 与 `Cube.001` 并存即 Fail),不得静默折叠 |
...
| ID | stage | order | impl | 适用 kind | writer | warning_codes |
|---|---|---|---|---|---|---|
| `r0.contract.schema_closed` | R0 | 10 | 1 | all | coordinator | — |
| `r0.contract.tools_locked` | R0 | 20 | 1 | all | coordinator | — |
| `r0.contract.na_set_declared` | R0 | 30 | 1 | all | coordinator | — |
| `r1.input.digest_recorded` | R1 | 10 | 1 | all | coordinator | — |
| `r1.input.no_link_or_device` | R1 | 20 | 1 | all | coordinator | — |
| `r1.input.size_within_limit` | R1 | 30 | 1 | all | coordinator | — |
| `r2.inventory.coverage_complete` | R2 | 10 | 1 | all | inspector | `unsupported_datablock_type`, `unsupported_modifier_type` |
| `r2.inventory.no_nan_inf` | R2 | 20 | 1 | all | inspector | — |
| `r2.inventory.no_reserved_props` | R2 | 25 | 1 | all | inspector | — |
| `r2.geometry.validate_clean` | R2 | 30 | 1 | all | inspector | — |
| `r2.geometry.manifest_written` | R2 | 40 | 1 | all | inspector | — |
| `r2.material.slots_resolved` | R2 | 50 | 1 | all | inspector | `empty_material_slot` |
| `r2.dependency.all_present` | R2 | 60 | 1 | all | inspector | `packed_dependency` |
| `r2.source.digest_stable` | R2 | 70 | 1 | all | inspector | — |
| `r3.export.file_nonempty` | R3 | 10 | 1 | interchange | export_glb | — |
| `r3.export.source_unchanged` | R3 | 20 | 1 | interchange | export_glb | — |
| `r3.validator.no_error` | R3 | 30 | 1 | interchange | coordinator | (glTF-Validator 原生码) |
| `r3.validator.resources_read` | R3 | 40 | 1 | interchange | coordinator | — |
| `r3.validator.report_complete` | R3 | 50 | 1 | interchange | coordinator | — |
| `r3.extension.none_forbidden` | R3 | 60 | 1 | interchange | coordinator | — |
| `r3.budget.within_limits` | R3 | 70 | 1 | interchange | glb_budget | `budget_near_limit`, `non_triangle_primitive` |
| `r4.reopen.offline_ok` | R4 | 10 | 1 | blend_native | reopen_probe | — |
| `r4.reopen.dependencies_resolved` | R4 | 20 | 1 | blend_native | reopen_probe | — |
| `r4.reopen.manifest_matches_source` | R4 | 25 | 1 | blend_native | reopen_probe | — |
| `r4.import.manifest_written` | R4 | 30 | 1 | interchange | reimport_probe | — |
| `r4.projection.preserved_fields_match` | R4 | 40 | 1 | interchange | coordinator | — |
| `r4.projection.transformed_within_tolerance` | R4 | 45 | 1 | interchange | coordinator | — |
| `r4.projection.undeclared_loss` | R4 | 50 | 1 | interchange | coordinator | — |
| `r4.projection.ambiguous_object_names` | R4 | 55 | 1 | interchange | coordinator | — |
| `r4.visual.scene_not_empty` | R4 | 60 | 1 | all | render_views(src) | — |
| `r4.visual.all_views_rendered` | R4 | 70 | 1 | all | coordinator | — |
| `r4.visual.self_determinism` | R4 | 80 | 1 | all | render_views(src) | — |
| `r4.visual.platform_key_known` | R4 | 90 | 1 | all | render_views(src) | — |
| `r4.visual.source_import_match` | R4 | 100 | 1 | interchange | coordinator | — |
| `r5.evidence.manifest_closed` | R5 | 10 | 1 | all | coordinator | — |
| `r5.evidence.hashes_match` | R5 | 20 | 1 | all | coordinator | — |
| `r5.contract.digest_stable` | R5 | 30 | 1 | all | coordinator | — |

**`writer` 列(冻结)**:声明该 check 的记录由哪一次进程调用写出。`coordinator` 表示由 coordinator 直接写入其自产 result(R3/R4)或 `summary.json`(R0/R1/R5)。`render_views(src)` 特指**第一次**渲染调用——三条 `r4.visual.*` 只由它写,第二次(import 侧)调用**只写 `view_imp_*` 图像与 `r4_render_imp_result`(其 `checks[]` 为空数组)**,从而不会与第一次撞 ID(§2.2 的"同一 stage 内不得出现同一 ID")。每个 check 有且仅有一个 writer;某 writer 未写出其应写的 check → 该 check 按 §2.5 第二步得 `Missing` 或 `Crash`。
...
| `imported_scene` | R4 | reimport_probe | `r4/imported.blend` | kind=interchange;**import 侧渲染的输入**,由 reimport_probe 在 fresh-import 后保存 |
| `projection_diff` | R4 | coordinator | `r4/projection-diff.json` | kind=interchange |
| `view_src_{view}_{pass}` | R4 | render_views(第一次调用) | `r4/views/src/{view}-{pass}.png` | 32 项(8×4);**source 侧** |
| `r4_render_src_result` | R4 | render_views(第一次调用) | `r4/render-src-result.json` | — |
| `view_imp_{view}_{pass}` | R4 | render_views(第二次调用) | `r4/views/imp/{view}-{pass}.png` | kind=interchange;**import 侧**,仅 `clay`/`silhouette` × 8 视角 = 16 项 |
| `r4_render_imp_result` | R4 | render_views(第二次调用) | `r4/render-imp-result.json` | kind=interchange |
| `render_settings` | R4 | render_views(第一次调用) | `r4/render-settings.json` | — |
...
- **每次子进程调用写自己的 result 文件**,不共用、不复写(`O_EXCL`):R2 一次(`r2_result`);R3 两次子进程(`r3_export_result`、`r3_budget_result`)加 coordinator 汇总 validator 的 `r3_result`;R4 `interchange` 三次子进程(`r4_probe_result`、`r4_render_src_result`、`r4_render_imp_result`)加 coordinator 的 `r4_result`,`blend_native` 两次(`r4_probe_result`、`r4_render_src_result`)加 `r4_result`。
- **`render_views.py` 被调用两次**(source 侧一次、import 侧一次),两次写不同的输出目录与 result 文件,故不违反 `O_EXCL`;import 侧那次以 `r4/imported.blend` 为输入(由 `reimport_probe` 在 fresh-import 后保存),渲染参数与 source 侧完全一致。
- **coordinator 自身产出的 `r3_result` / `r4_result`** 承载"外部工具结果的汇总"类 check(validator、oiiotool 比较、投影 diff),其 `stage` 字段照填。它们没有子进程,故条款 9 的 "exit == 0" 部分对其不适用,但**其 schema 与 `success == true` 仍受条款 9 校验**(条款 9 的后半句写的是"所有 result 文件"),`O_EXCL` 同样适用。
- 同一 stage 内各 result 的 `checks[]` **不得出现同一 ID**(重叠 → `expected_set_mismatch`);该 stage 的 actual check 集 = 各 result 的并集 ∪ coordinator 合成的 N/A 记录。§2.6 条款 9 的"其产物"即该次子进程调用自己的 result 文件。
...
**第一分类:失败必须先分两类**——`check_failed`(资产本身被拒收,验收流程正常完成)与其余 15 个基础设施 family(流程未能正常完成)。`summary.failure_code` 取值规则:
...
tolerated_unknown_types: [str]                     # 允许以 warning 形式出现的未分类 datablock 类型,默认 []
...
```text
schema_version: int = 1
stage: "r2"|"r3"|"r4"          # R0/R1/R5 为 coordinator-owned,不产生 result 文件(§7.1.1)
success: bool                          # 语义见 §2.1
contract_digest: str
checks: [{id: str,                 # 无 raw_status、无 disposition —— 二者由 coordinator 计算(§2.5)
          #   保留键名:出现 "raw_status" 或 "disposition" → forged_disposition(见下)
          tool_id: str|null, tool_version: str|null,
          findings: [{code: str, severity: "error"|"warning"|"info",
                      pointer: str|null, offset: int|null, detail: str|null}],
...
| `acceptance` | 本仓验收包自身 | 全部自研 check 的 `tool_id` | **覆盖范围(冻结)**:`acceptance/` 下全部 `.py` 与 `.json`(含 `schemas/`、`blender_scripts/`)加 `scripts/asset_accept.py`;按仓库相对路径字典序,逐文件 SHA-256,再对 `路径\n哈希\n` 序列取 SHA-256。`version` = `acc-<前 12 位十六进制>`;`sha256` = 该聚合哈希全值;`path` = `acceptance`(包目录,非单文件——这是本表 hash 边界规则的唯一例外,因为它是被 import 的包而非被执行的二进制) | coordinator 启动时自算并写入 `summary.runner_provenance.acceptance_files`;与合同锁定值不符 → `toolchain_mismatch`。合同作者在 R0 前即可计算该值,不构成循环依赖 |
...
**GLB 导出 preset(冻结,`contract.export.preset`)**:`export_format='GLB'`、`export_apply=True`(应用 modifier)、`export_yup=True`(导出器默认,与导入端对称,见 §6)、`export_draco_mesh_compression_enable=False`、`export_image_format='AUTO'`、`export_jpeg_quality=100`、`export_texture_dir=''`(内嵌)、`export_cameras=False`、`export_lights=False`、`export_animations=False`(P0 静态)、`export_extras=True`(承载 `bcx_uid`,见 §6 `p09`)、`export_skins=False`、`export_morph=False`、`export_texcoords=True`、`export_normals=True`、`export_tangents=False`、`export_materials='EXPORT'`;**对象过滤(冻结,消除两端边界分歧)**:`use_selection=False`、`use_visible=True`、`use_renderable=True`、`use_active_collection=False`。以上参数名均经 Blender 5.2 实测存在(`bpy.ops.export_scene.gltf.get_rna_type().properties`)。KTX2/basisu 与 `KHR_animation_pointer` 在此 preset 下不产生;`r3.extension.none_forbidden` 另做出后校验。
...
3. 至少一个生成器 known-good 全绿(视觉证据按 kind 齐备:`blend_native` 32 张、`interchange` 48 张 + 16 张差异图,且自确定性通过);
...
| `projection_union_incomplete` | L0 | synthetic | `contract_invalid`(三数组并集 ≠ `p01`…`p14`) | 无 |
...
| `collection_instance_roundtrip` | L0 | generator | **通过**——一个被 collection-instance 实例化 N 次的对象必须通过 `p01`/`p02`/`p09`;专防"重复 uid 即 Fail"型误拒(第五处) | 无 |
| `hidden_render_object_excluded` | L0 | generator | **通过**——含 `hide_render=True` 对象的资产必须通过 `p01`;两端过滤规则一致(§4.1 与 §7.4 preset) | 无 |
...
| Fixture | 预期 |
|---|---|
| `allowlisted_warning_accepted` | 四元组命中 allowlist 的 warning finding → coordinator 计算出 `raw_status=Pass`、`effective=Pass`、`accepted=true`;**该 finding 的 `severity` 仍为 `warning`、`disposition=AcceptedWarning` 原样保留**(原始证据不被改写的落点是 finding,不是 check 层的 raw_status) |
...
### 12.5 V3.8 收敛验证(第七轮)处置

第七轮发现 2 High / 4 Medium / 4 Low(较第六轮 3H/7M/5L 收敛),**全部成立**并已修复。审计判定:修完这几处后 V3.8 **可作为 P0 可执行规范基线**。

| 发现 | 验证 | 处置 |
|---|---|---|
| H-1 §2.5 第二步阶梯顺序错:`Missing` 遮蔽 `NotApplicableByContract` 与 `Crash` | 成立(照冻结表实现即得"任何资产永不放行";`tool_killed_by_signal` 夹具期望的 `tool_crashed` 会变成 `evidence_missing`) | 顺序改为 `N/A → Crash → Missing → Truncated → Fail → Warning → Pass`,并写明两条排序理由 |
| H-2 §4.1 的"三方一致"不成立,第六处误拒开放 | 成立(`use_visible` 谓词未覆盖、collection 级可见性未覆盖、`VOLUME`/`POINTCLOUD`/`GREASEPENCIL` 在 glTF 无表示却计入 `p01`) | 拆为**两套过滤集**:`render_set`(供包围盒与渲染)与 `export_set`(供 `p01`/`p02`,再加 glTF 可表示类型与 `visible_get()` 两条);manifest 同时输出两个列表使对账口径显式;`export_set` 为空且 kind=interchange 时由 `r3.export.file_nonempty` 报 `no_exportable_geometry` |
| M-1 `forged_disposition` 与已关闭的 result schema 相撞 | 成立(字段既已不在 schema 内,写出即归 `tool_output_invalid`,优先级压过 `forged_disposition`) | §7.3 声明 result 封闭且未知键归 `tool_output_invalid`,**唯二例外**是保留键名 `raw_status`/`disposition` 归 `forged_disposition` |
...
V3.7 终验发现 3 High / 7 Medium / 5 Low,**全部成立**并已在本版修复。

| 发现 | 验证 | V3.8 处置 |
|---|---|---|
| H-A `raw_status` 归属三重矛盾(§2.5 vs §8.4 夹具 vs §7.3 schema vs N/A 合成) | 成立(`decide.py` 无法无歧义编码) | §2.5 定案统一表达形式:**每个 check 的结果就是一组 finding**;子进程只写 `findings[]` 与 `source_truncated`,`raw_status`/`disposition` 由 coordinator 独占计算;第二步补齐 `Missing`/`Crash`/`Truncated`/`NotApplicableByContract` 四个终结态的产生点;result schema 删除 `raw_status`;§8.4 夹具改为 `raw_status=Pass` 而 finding 的 `severity`/`disposition` 原样保留(原始证据的落点是 finding,不是 check 层) |
| H-B transformed 类字段违规无 owner → fail-open | 成立(`p04` 比值爆炸 10× 也全程 Pass) | 新增 check `r4.projection.transformed_within_tolerance`(R4/order 45/interchange);§6 首段明确四类判定分工,**无字段处于无 owner 状态** |
| H-C `p09` 唯一性与 `instances[]` 相撞(第五处误拒) | 成立(M-04 要保护的实例化资产被 H-06 规则无条件拒收) | `p09` 改为**多重集**比较:期望多重集由 `instances[]` 计算(对象被实例化 N 次即合法出现 N 个相同 uid),`A == E` 为 Pass;缺失仍 Fail。新增正向夹具 `collection_instance_roundtrip` |
| M-A 并集 13 vs 14 | 成立 | §6 首段与夹具 `projection_union_incomplete` 同步为 `p01`…`p14` |
| M-B `result.json.stage` 值域未同步 | 成立 | §7.3 改为 `"r2"\|"r3"\|"r4"` |
| M-C `all_views_rendered` 基数二义 | 成立 | §5.2-1 与 §7.8-3 同步为按 kind 取 32 / 48;差异图不计入本 check |
| M-D interchange R4 进程拓扑无解 | 成立(`render_views` 无法启动两次、imported scene 未落盘) | §7.1.1 新增 `imported_scene`(`r4/imported.blend`)与两份渲染 result(`r4_render_src_result` / `r4_render_imp_result`);明确 `render_views.py` 调用两次、写不同目录与 result,不违反 `O_EXCL` |
| M-E `p01`/`p02` 对象集合边界未冻结(潜在第六处误拒) | 成立 | §4.1 `InstanceRecord` 补**过滤规则**(可渲染类型 + 非 `hide_render`),与 §5.1 bounds 遍历、§7.4 preset 的 `use_visible=True, use_renderable=True` **三方一致**;新增正向夹具 `hidden_render_object_excluded` |
| M-F `instancer_stable_id` 对真实对象无取值 | 成立 | 定为空串 `""`,`persistent_id` 取 `[]`,使排序键与 digest 可复算 |
| M-G 自研 warning 五个 vs 六个 | 成立 | §2.5 改为"以注册表 `warning_codes` 列为准",并列出当前六个 |
| L-A/L-B family 计数回归与覆盖矩阵漏项 | 成立(连续第二轮同形态复发) | 计数与覆盖矩阵改为**从表格自动重算**后写入;§7.8-6c 同步。**教训**:每轮新增枚举项后必须重算全部计数,已列入定稿检查 |
| L-C "唯一取 authored"被 `p14` 证伪 | 成立 | 改为"`p11` 与 `p14` 取 authored" |
| L-D 仅有 info finding 时误置 `accepted=true` | 成立 | 第二步改为"若原本存在至少一条 warning 且全部被接受"才置位 |
| L-E `acceptance` 行的 `sha256`/`path` 未定;`assumption_unverified` 无落点 | 成立 | §7.4 冻结其覆盖范围(`acceptance/` 全部 `.py`/`.json` + `scripts/asset_accept.py`)、`sha256` 取聚合哈希全值、`path` 取包目录并声明为 hash 边界规则的唯一例外;`assumption_unverified` 写入该 check 的 `metrics` 并追加同名 advisory |

终验同时确认**不成立**的一项疑虑,记录以免重复质疑:`acceptance` 版本(包内容哈希)与合同冻结**不构成循环**——哈希是代码在磁盘上的属性,合同作者在 R0 前即可计算并填入,R0 由 coordinator 自算比对;依赖方向单向。

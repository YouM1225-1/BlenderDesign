...
自研 `blender-codex-server` 使用 `mcp==2.0.0` 和 `mcp.server.MCPServer`。协议适配集中在 `server/mcp/adapter.py`，core 层不依赖 MCP SDK。
...
- 当前封闭输入 schema 的 workaround 访问 SDK 私有 `ToolManager` 状态，因此在改用公开 API 前不宣称兼容未经验证的 2.x 版本；
- 只维护一套 adapter 能减少协议分叉和重复测试。

## 约束

- Python 保持 `>=3.13,<3.14`；
- SDK 精确版本由 `pyproject.toml` 和 `uv.lock` 双重固定；

...
| proposed manifest | `2026-08-08-r16-proposed-plan-python-manifest.tsv`、`2026-08-08-r17-proposed-plan-python-manifest.tsv` | 隔离物化候选，不是实施产物；r16 已获批但在提交前被 r17 supersede；一次旧 r17 tuple 批准也因 live 字节不匹配而未绑定 | 当前 r17 精确 tuple 审批/提交后仍保留两版以解释审批链 |
...
| GUI smoke | `2026-08-07-phase0-gui-smoke-closeout-v2.json`～`v8.json` | 真实 Blender GUI 的当轮基础判据；不是 r17 正式 NFR/recovery | v2～v5、v6～v7 各自 byte-identical，文件名仍承载 run/revision |
...
3. 正式 Phase 0 artifact 只能在获批 r17 tuple、`source_commit → attestation commit` 两提交链和执行前机械验证完成后生成。

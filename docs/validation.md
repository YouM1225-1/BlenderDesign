...
2026-09-24 正常用户 profile 已修复并完成现场清理，上文该 profile 的 `exact=false` 与未维修结论由本段取代。此前修复工作流 `1e522b8d-442b-4a01-a465-aa3fd2fa8e98`（receipt `02adb067`）的 inspect、live VERIFY 与注册验证均已通过，但 FINALIZE 只返回通用 `installer error`，journal 停在 `cleanup_pending`。原因是 26 条旧注册 `before.json` 指向已整体删除的 projection，读取其 manifest 失败使受保护引用计算中止。`e31c3d7` 改为：整体缺失的 projection 只保护 source 路径；仍存在但无法证明的旧注册 source 引用以固定原因 `cleanup_reference_unproven` 失败关闭，不输出路径、不改写证据。新增 11 个行为回归，缺失 projection 用例在未修复代码上复现同一错误；完整门禁 1070 passed / 1 skipped，`ALL CHECKS PASSED`；三轮独立只读审查无阻塞项。

现场使用 Codex 0.155.0-alpha.16、Blender 5.2.0、Python 3.13.13、uv 0.12.2，受信提交 `e31c3d7`。inspect 为 `exact=true`。首次 install 在任何注册修改前以 exit 1 结束：ChatGPT app 的 codex 进程启动的 7 个托管 MCP 进程持有 runtime 共享使用锁；本次只物化了可复用的 `e31c3d7` projection，没有 journal、receipt、active 或 Codex 配置变化。操作者正常退出 ChatGPT 后重试 installer-version-only install，结果为 `no_op=true`，工作流 `caa64e2e-6f0f-4ea5-abfd-85133a6bca79`。操作者重新打开 Blender 后，VERIFY 的 26 工具目录与 Blender 只读调用通过；FINALIZE 以 exit 3 / `cleanup_pending` 结束，删除旧插件 cache `1.0.0+codex.20260922221552`，4 个旧 recovery 候选因 `lease_known=false` 记为 `deferred_in_use`（`legacy usage is not proven idle`）；PERSISTENT_MARKETPLACE_VERIFY 通过。`1e522b8d` 绑定 `1ac39ea`，以 `e31c3d7` 重试在修改前失败（通用 `installer error`，按代码为 finalize identity mismatch），journal 字节不变，保持 `cleanup_pending`；其候选已由 `caa64e2e` 继承。11 个 runtime recovery（约 1.3 GB，含扩展 recovery）仍保留：4 个无租约旧候选不能证明空闲，其余为 `preimage lacks exact managed parent provenance` 发现项；清理保护未放宽，回收方案另行处理。证据位于 `/private/var/tmp/blenderdesign-normal-profile-evidence-20260924/e31c3d7`，`SHA256SUMS` 的 SHA-256 为 `b076c4dd747d4917d2f4fbf20545f923eca00905c524b6f130adcc9fd28b7e9b`。严格 RELEASE、LLM 与第二台 Mac 限制不变。
